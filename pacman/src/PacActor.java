...
public class PacActor extends Actor implements GGKeyRepeatListener
{
  ...
  private boolean lastLocPortal = false;
  ...
  public void act()
  {
    Location moveto =new Location(-1,-1);
    Location nonVaild = new Location(-1,-1);
    for(Portal portal:game.portals) {
      if (this.getLocation().equals(portal.getLocation())) {
        moveto = portal.getLocationMoveTo();
      }
    }
    if(!moveto.equals(nonVaild)&&lastLocPortal==false) {
      this.setLocation(moveto);
    }

    lastLocPortal = isLocPortal(this.getLocation());
    show(idSprite);
    idSprite++;
    ...
    for(int i = 0;i<game.getPillAndItemLocations().size();i++){
      if(this.getLocation().equals(game.getPillAndItemLocations().get(i))){
        game.removePill(i);
      }
    }
  }
  ...
  private boolean isLocPortal(Location location) {
    for (Portal portal : game.portals) {
